...
hncp_tlv_trust_verdict(const struct tlv_attr *a)
{
  ...
  if (tlv_len(a) < sizeof(hncp_t_trust_verdict_s) + 1)
    return NULL;
  if (tlv_len(a) > sizeof(hncp_t_trust_verdict_s) + DNCP_T_TRUST_VERDICT_CNAME_LEN)
    return NULL;
...
hncp_node_find_neigh_bidir(hncp_node n, hncp_t_node_data_neighbor ne)
{
  ...
  hncp_node_for_each_tlv_with_type(n2, a, DNCP_T_NODE_DATA_NEIGHBOR)
...
{
  hncp_tlv t = hncp_find_tlv(l->hncp, DNCP_T_NODE_DATA_NEIGHBOR,
                             n, sizeof(*n));
  if (t)
